...
import p2pserver.PeerServer.HandleClients;
...
public class DirectoryServer extends JFrame {
	 ...
	 * This one act only on UDP, for now all it does is print out the name of the new user, one at a time
	 */
	public class ServerStartWaitForPacket implements Runnable {
		...
		public void run() {
			try {
				/*
				 * TODO This thing thought an exception cuz it tries it over and over again
				 * so what needs to be done is spDHlit this try and the one inside in to two separate classes
				 * this way you establish connection once but wait for receive(packet) many times
				 */
				 datagramSocket = new DatagramSocket(port);
				//datagramSocket.setSoTimeout(10000); //time out was in one of the question... thought it would be good to include
	...
	public void userUpdate(String[] dataTotal) {
		String online = "ON", offline = "OF", listAllUsers = "LI", hostingAServer = "HO", doneHostingAServer = "DH", joinServer = "JO", leaveServer = "LS";
		...
		//	System.out.println(dataTotal[2].split(":"));
			...
			comArea.append("Hosting: " + dataTotal[2] + ":" + dataTotal[3] + ":" + dataTotal[4] + ":" + dataTotal[5] + "\n");
			for (int i = 2; i <dataTotal.length; i++){
			cheathostUsers.add(dataTotal[i]);
					}
		
			hostUsers.add(dataTotal[2].trim() + ":" + dataTotal[3].trim() +  ":" + dataTotal[4].trim() + ":" + dataTotal[5].trim());
			...
			for(int i=0; i<hostUsers.size();i++){
				...
				int rate = Integer.parseInt(temp.substring(temp.lastIndexOf(dataTotal[4].trim())+dataTotal[4].trim().length() + 1));
				rate--;
	...
	private void tellEveryoneAllOnlineUsers() {
		...
		String allHostInOneString = "DI:HO";
			...
			try {
					...
					byte[] buffer2 = allUserInOneString.getBytes();

					byte[] buffer3 = allHostInOneString.getBytes();
					InetAddress receiverAddress = InetAddress.getLocalHost();
					
					DatagramPacket packet = new DatagramPacket(buffer2, buffer2.length, receiverAddress, cliport);
					DatagramPacket packet2 = new DatagramPacket(buffer3, buffer3.length, receiverAddress, cliport);
	...
	public void sendUpdatedUsers() {
		// sends message to everyone connected to server
		for(int i=0; i<onlineUsers.size(); i++){
			comArea.append(onlineUsers.get(i) + "\n");
		}
