...
import p2pserver.PeerServer;

import dirserver.DirectoryServer.ServerStartWaitForPacket;
...
public class ChatMain extends javax.swing.JFrame {
	 ...
	 */
	public String setIpAddress() throws UnknownHostException {
		InetAddress ip;
		String serverIP;
		try {
	...
	private void processPacket(byte[] buffer) {
		...
		String[] dataTotal = messageReceived.split(":");
	//	System.out.println(dataTotal[0].trim());
		if (directoryMessage.equals(dataTotal[0].trim())) {
			...
			//comArea.append("No Conditions were met, the message was not from the Client or Server. \n");
		}
	...
	//	textAllUsersOnline.append("Received:" + dataTotal[0] + "," + dataTotal[1] + "," + dataTotal[2] + "\n");
	...
	public void initRoomToJoin() {
		...
		/* Host Users - line 4 
		 * This one is temporary for now, since all it does is join the same server
		 */
...
*/
	...
	private void sendButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if ((inputTextArea.getText()).equals(nothing)) {
			...
			try {
				writer.println(username + ":" + inputTextArea.getText() + ":"
						+ "Chat");
				writer.flush(); // flushes the buffer
			} catch (Exception ex) {
